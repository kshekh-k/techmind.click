...
export default function TextFormatter() {
  ...
  const [isUnicodeBold, setIsUnicodeBold] = useState(false);
  const [isUnicodeItalic, setIsUnicodeItalic] = useState(false);
  ...
  function applyUnicodeFormatting(
    input: string,
    bold: boolean,
    italic: boolean,
  ) {
    if (bold && italic) {
      return convertUnicode(input, "boldItalic");
    }

    if (bold) {
      return convertUnicode(input, "bold");
    }

    if (italic) {
      return convertUnicode(input, "italic");
    }
  ...
  const handleUnicodeBold = () => {
    const nextBold = !isUnicodeBold;
    setIsUnicodeBold(nextBold);

    const formattedText = applyUnicodeFormatting(
      sourceText,
      nextBold,
      isUnicodeItalic,
    );
  ...
  const handleUnicodeItalic = () => {
    const nextItalic = !isUnicodeItalic;
    setIsUnicodeItalic(nextItalic);
  ...
  const applyText = useCallback(
    (
      nextValue: string,
      nextListType: ListType = activeListType,
      cursorPos?: number,
    ) => {
  ...
  const handleSpellCheck = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setSpellErrors([]);
        setSpellCheckMessage(
          data.message ?? "Spell check failed. Please try again.",
        );
        return;
      }
      ...
      setSpellCheckMessage(
        matches.length === 0 ? "No spelling issues found." : "",
      );
    } catch {
      setSpellErrors([]);
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
      console.log("KEYDOWN", event.key, activeListType);
      ...
      if (event.key === "Enter" && activeListType !== "none") {
        event.preventDefault();
        ...
        if (
          ...
          const cleaned = beforeCursor.replace(/(\n?)(\s*)(•|○|\d+\.)\s?$/, "");
          const nextText = cleaned + afterCursor;

          setText(nextText);
          setOriginalText(nextText);
          setActiveListType("none");
        ...
        if (activeListType === "bullet") {
          marker = "• ";
        } else if (activeListType === "circle") {
          marker = "○ ";
        } else if (activeListType === "number") {
          const match = currentLine.match(/^(\s*)(\d+)\.\s/);
          const nextNumber = match ? Number(match[2]) + 1 : 1;
          marker = `${nextNumber}. `;
        }
        ...
        requestAnimationFrame(() => {
          if (textAreaRef.current) {
            const nextPos = cursorPos + marker.length + 1;
    ...
    [activeListType, pushHistory, redo, resetSpellState, undo],
  );
    ...
    (action: FormatAction) => {
      ...
      switch (action) {
        ...
        case "sentencecase":
          nextText = text
            .toLowerCase()
            .replace(SENTENCE_CASE_PATTERN, (char) => char.toUpperCase());
          break;
        case "bullet-list":
          nextText = formatList(text, "bullet", activeListType);
          nextListType = nextText === text ? "none" : "bullet";
          console.log("SET BULLET:", nextListType);
          break;
        case "number-list":
        ...
        case "clear":
          ...
          setIsUnicodeBold(false);
          setIsUnicodeItalic(false);
          setOriginalText("");
      ...
      if (action === "bullet-list" || action === "circle-list") {
        addedChars = 2;
      } else if (action === "number-list") {
        addedChars = 3;
      }

      applyText(nextText, nextListType, cursorPos + addedChars);
    },
          ...
          <div className="flex justify-between flex-wrap gap-2 pb-2">
            <div className="flex flex-wrap gap-2">
              ...
              <div
                className="flex"
                role="group"
                aria-label="Text Format Unicode"
              >
                <Button
                  variant={"outline"}
                  className="rounded-r-none border-r-0"
                  aria-label="Bold"
                  onClick={handleUnicodeBold}
                >
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => formatText("reverse-text")}
              >
                Reverse Text
              </Button>
            </div>
